...
func (m *Master) handle_new_worker() {
	idx := 0
	for {
		m.Mu.Lock()
...
func (m *Master) master_start_server() {
	rpcs := rpc.NewServer()
	err := rpcs.Register(m)
	if err != nil {
...
func (m *Master) master_shutdown_server() {
...
func makeMaster(name string) *Master {
	...
	m.workers_chan = make(chan string)
...
func RunMaster(files []string, nReduce int, master_name string, job_name string, return_chan chan *Master) *Master {
	...
	go m.handle_new_worker()
	go m.master_start_server()
	...
	for _, worker := range m.workers {
		...
		ok := call(worker, "Worker.Worker_shutdown_server", new(struct{}), &reply)
		if ok == false {
	...
	for i, num := range worker_task_gather {
		fmt.Printf("%s: Worker %d has finished %d tasks\n", m.Master_name, i, num)
	}
	// shutdown Master server
	m.master_shutdown_server()
